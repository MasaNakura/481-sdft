...
# CHANGE: trl>=0.28 moved VLLMClient and dropped some re-exports — use trl_compat_new.
from trl_compat_new import (
    VLLMClient,
    apply_chat_template,
    is_conversational,
    prepare_multimodal_messages,
    trl_maybe_apply_chat_template as _trl_maybe_apply_chat_template,
    profiling_context,
    profiling_decorator,
...
from qwen_compat_new import (
    apply_chat_template_compat,
    build_vllm_llm,
    filter_ctor_kwargs,
    is_qwen3_family,
    merge_model_init_kwargs,
    model_generate_kwargs,
    tokenizer_uses_enable_thinking,
)


def _processing_tokenizer(processing_class):
    if hasattr(processing_class, "apply_chat_template"):
        return processing_class
    return processing_class.tokenizer


def _vllm_worker_load_weights(self, weights):
    """
    # CHANGE: vLLM v1 (>=0.10) engine has no `model_executor` attribute on LLMEngine;
    weight sync must go through collective_rpc. This module-level function is what
    gets invoked inside each vLLM worker process — `self` here is the worker.
    """
    self.model_runner.model.load_weights(weights)


def maybe_apply_chat_template(example, processing_class, enable_thinking: bool = False):
    """
    # CHANGE: route Qwen3.x-style prompts through apply_chat_template_compat using the
    `enable_thinking` content check (not `name_or_path` matching).

    Earlier revisions gated on `is_qwen3_family(tokenizer.name_or_path)`. That works during
    SDFT training (tokenizer loaded from `Qwen/Qwen3.5-…`) but BREAKS at eval and at any
    intermediate checkpoint reload, where `name_or_path` is the user's save dir and almost
    never contains "qwen3". The fall-through to TRL's `_trl_maybe_apply_chat_template`
    silently rendered a thinking-on prompt even when the caller asked for
    enable_thinking=False — same train↔eval mismatch that tanked SFT, but now also at
    SDFT training time when the teacher's vLLM colocated tokenizer was loaded from a
    non-canonical path.

    Detecting `enable_thinking` in the saved chat-template string is invariant to local
    paths, so the same code path works for hub models AND saved checkpoints.
    """
    tok = _processing_tokenizer(processing_class)
    if tokenizer_uses_enable_thinking(tok) and "prompt" in example:
    ...
    return _trl_maybe_apply_chat_template(example, processing_class)
...
class DistilTrainer(BaseTrainer):
    """
    Trainer for the Self-Distillation method (Qwen3.5 / transformers>=5 / vLLM>=0.13 variant).

    See qwen_compat_new.py for API adaptations vs distil_trainer.py. 
    ...
    def __init__(
        ...
        # Models
        # Trained model
        # CHANGE: merge Qwen3.5-safe defaults (dtype, trust_remote_code) into model_init_kwargs.
        model_init_kwargs = merge_model_init_kwargs(args.model_init_kwargs, model if isinstance(model, str) else model.config._name_or_path)
        if isinstance(model, str):
            ...
            # CHANGE: trust_remote_code required for Qwen3_5* Hub checkpoints.
        ...
        self.enable_thinking = getattr(args, "enable_thinking", False)
        ...
        # CHANGE: Qwen3.5 (Qwen3_5ForConditionalGeneration) has no warnings_issued; skip when absent.
        ...
        if ref_model is not None:
            ...
            # CHANGE: trust_remote_code for Qwen3.5 reference / teacher weights.
            config = AutoConfig.from_pretrained(model_id, trust_remote_code=True)
            architecture = getattr(transformers, config.architectures[0])
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                # CHANGE: build_vllm_llm wraps LLM() for Qwen3.5 (reasoning_parser, dtype) and filters unsupported kwargs.
                vllm_extra = {}
                if self.enable_thinking:
                    vllm_extra["default_chat_template_kwargs"] = {"enable_thinking": True}
                self.llm = build_vllm_llm(
                    vllm_model_path,
    ...
    def _load_vllm_weights(self, weights):
        """
        # CHANGE: replaces direct `llm.llm_engine.model_executor.driver_worker.model_runner.model`
        access (removed in vLLM v1 engine). Falls back to legacy attribute path when available
        so behavior on vLLM<0.10 (and external_launcher single-process setups that still expose
        model_executor) is unchanged.
        """
        weights = list(weights)
        executor = getattr(self.llm.llm_engine, "model_executor", None)
        if executor is not None:
            try:
                llm_model = executor.driver_worker.model_runner.model
                llm_model.load_weights(weights)
                return
            except AttributeError:
                pass
        # vLLM v1 engine: dispatch via collective_rpc to each worker.
        self.llm.collective_rpc(_vllm_worker_load_weights, args=(weights,))
            ...
            with FSDP.summon_full_params(module, recurse=False, writeback=False):
                for param_name, param in module.named_parameters():
                    ...
                    if self.vllm_mode == "server" and self.accelerator.is_main_process:
                        ...
                        self._load_vllm_weights([(full_name, param.data)])
    ...
    def _sync_fsdp2_params_to_vllm(self, module: nn.Module):
        ...
        for name, param in module.state_dict().items():
            ...
            if self.vllm_mode == "server" and self.accelerator.is_main_process:
                ...
                self._load_vllm_weights([(name, param)])
                ...
                if self.is_fsdp_enabled:  # note if using FSDP, gather_if_zero3 is nullcontext
                    ...
                    for name, param in self.model.named_parameters():
                        ...
                        if self.vllm_mode == "server" and self.accelerator.is_main_process:
                            ...
                            self._load_vllm_weights([(name, param.data)])
    ...
    def _generate_single_turn(self, prompts: list[str], images: Optional[list]):
        ...
        prompts_text = [
            maybe_apply_chat_template({"prompt": prompt}, self.processing_class, self.enable_thinking)["prompt"]
        ...
        if self.use_vllm:
            ...
            # Generate completions using colocated vLLM instances: each device holds vLLM copy and work on their own batch of prompts
                ...
                generation_kwargs = {
                    ...
                    "max_tokens": self.max_completion_length,
                    "truncate_prompt_tokens": self.max_prompt_length,
                    # CHANGE: vLLM>=0.21 treats logprobs=0 as falsy (== None) — no logprobs are
                    # returned and output.logprobs comes back as None. Ask for top-1 instead; the
                    # sampled token's logprob is always included even when it isn't the top-1
                    # token, and we look it up by token id below to stay order-independent.
                    "logprobs": 1,
                }
                if self.args.generation_kwargs is not None:
                    generation_kwargs.update(self.args.generation_kwargs)
                # CHANGE: vLLM>=0.21 removed `truncate_prompt_tokens` (and possibly other legacy
                # fields) from SamplingParams. Drop kwargs the installed SamplingParams doesn't
                # accept so prompt-length truncation simply becomes a no-op on newer vLLM
                # (callers are expected to truncate prompts themselves when needed).
                filtered_kwargs = filter_ctor_kwargs(SamplingParams, generation_kwargs)
                dropped = set(generation_kwargs) - set(filtered_kwargs)
                if dropped:
                    import warnings
                    warnings.warn(
                        f"Dropping SamplingParams kwargs unsupported by this vLLM build: {sorted(dropped)}",
                        RuntimeWarning,
                        stacklevel=2,
                    )
                sampling_params = SamplingParams(**filtered_kwargs)
                ...
                # CHANGE: look up the sampled token's logprob explicitly by token id rather
                # than `next(iter(lp.values()))`. vLLM's dict ordering for the sampled vs.
                # top-k entries has shifted across versions; indexing by token id is stable.
                all_logprobs = [
                    [lp[tid].logprob for tid, lp in zip(output.token_ids, output.logprobs)]
            ...
            with (
                ...
                # CHANGE: disable_compile removed in some transformers>=5 models; pass only if supported.
