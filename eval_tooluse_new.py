...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a model on tooluse test set (new stack)")
    ...
    # CHANGE: expose the full Qwen3.5 recommended sampling surface; defaults mirror vLLM
    # SamplingParams defaults (neutral, no behavior change for existing commands). Per the
    # Qwen3.5-2B model card, non-thinking text tasks should use
...
def _collect_stop_token_ids(tokenizer) -> list[int]:
    ...
    # `tokenizer.eos_token_id`, missing the second one.
    """
...
def generate_responses(
    ...
    # Sanity log so user can verify the chat-template tail matches training.
    if prompts:
        tail = prompts[0][-200:]
        print("=" * 60)
